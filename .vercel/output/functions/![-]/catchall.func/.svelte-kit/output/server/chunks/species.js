...
function searchSpecies(q) {
  ...
  return SPECIES.map((s) => {
    ...
    const rank = s.name.toLowerCase().startsWith(needle) ? 0 : s.name.toLowerCase().includes(needle) ? 1 : (s.aka ?? []).some((a) => a.toLowerCase().includes(needle)) ? 2 : 3;
    return { s, rank };
  }).filter((x) => x !== null).sort((a, b) => a.rank - b.rank || a.s.name.localeCompare(b.s.name)).map((x) => x.s);
}
export {
  SPECIES as S,
  searchSpecies as a,

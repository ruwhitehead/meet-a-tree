...
<nav aria-label="Main">
	{#each items as item (item.href)}
		<a class="nav-btn" href="{base}{item.href}" aria-current={isActive(item.href) ? 'page' : undefined}>
			{@render icon(item.icon)}{item.label}
		</a>
	{/each}
	<div class="fabwrap">
		<a class="fab" href="{base}/identify" aria-label="Identify a tree" aria-current={isActive('/identify') ? 'page' : undefined}>

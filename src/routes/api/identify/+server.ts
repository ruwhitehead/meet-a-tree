...
/** Species we carry, keyed by the scientific names Pl@ntNet returns. */
...
/** Proxies one photo to Pl@ntNet. The API key stays on the server — this is the
 *  whole reason the endpoint exists rather than calling Pl@ntNet from the page. */
export const POST: RequestHandler = async ({ request, fetch }) => {
	...
	const url =
		`https://my-api.plantnet.org/v2/identify/k-world-flora?api-key=${encodeURIComponent(key)}` +
		'&include-related-images=false&no-reject=false&nb-results=5&lang=en';
	...
	if (res.status === 404) return json({ ok: true, matches: [] }); // nothing recognised
	...
	const data = (await res.json()) as {
		results?: { score: number; species?: { scientificNameWithoutAuthor?: string; commonNames?: string[] } }[];
	};
